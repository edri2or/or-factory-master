...
| `oil-autofix-investigate.yml` | `repository_dispatch[oil-investigate]` (from MCP triage) + manual `workflow_dispatch(issue_id)` | **OIL auto-fix, stages 1–3.** Reads an OIL Linear ticket + the failed run's logs, runs a READ-ONLY investigator (`claude-code-action`, Read/Grep/Glob; `allowed_bots`=broker only) → diagnosis JSON; if `actionable-bug` (confidence ≥ 0.8) in `or-factory-master`, a fixer writes a small fix + a bash reproducer, a deterministic gate (`scripts/oil-autofix-validate.sh`) proves it (≤2 files/≤100 lines, no forbidden paths/secrets, fail-before/pass-after in scrubbed `env -i`, after cloud creds are revoked), and a **DRAFT** PR is opened by the broker App + registered for Telegram ✅/❌ approval. Never merges (stages 4–5 do). On the `dispatch_workflow` allowlist. Full reference: `docs/oil-autofix.md`. |
| `oil-autofix-verify.yml` | `push: main` + manual `workflow_dispatch(pr_number/issue_id/test_cmd)` | **OIL auto-fix, stage 5 — post-merge verify + auto-close.** Acts only on an OIL merge (recognised by the `oil-autofix:` squash subject), recovers the reproducer from the merge diff, and re-runs it on merged `main` via `scripts/oil-verify.sh` (scrubbed `env -i`, after revoking cloud creds). PASS → Linear `issueUpdate(completed)` + closing comment + "✅ תוקן ואומת" Telegram; FAIL → comment + "🚨 נכשל באימות" Telegram, issue stays OPEN. Holds **no** GitHub API token in its logic (issue/PR from the commit subject, reproducer from the merge diff; Linear/Telegram via WIF→SM). **Not** on the `dispatch_workflow` allowlist (it acts only on already-merged `main` commits). Full reference: `docs/oil-autofix.md`. |
