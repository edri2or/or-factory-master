...
> **עדכון 2026-07-19:** לבקשת Or הוחזרה יכולת רזה וממוקדת להקמת מערכת-אחות אחת נקייה — `/new-system`
> (`bootstrap-system-infra.yml` + `templates/new-system/`). הוכחה ראשונה: `or-agents`. זו **לא** החייאת המכונה — playbook קבוע; ראה "The fold".
...
| `notify-routine-ci-green.yml` | `workflow_run` on the 5 CI workflows (`Changelog Check` / `Pipeline Tests` / `Secret Scan` / `Supply Chain Check` / `Playground Tests`) + manual | Wakes a Claude Code session subscribed to a PR when CI goes green. A subscribed session is pushed CI *failures* and review comments but **not** CI *success*, so a green PR never wakes it on its own; a submitted PR *review* **is** pushed. On CI completion this re-reads every check-run on the PR head commit and, only when they are **all** green, posts a short review (`event=COMMENT`, default `GITHUB_TOKEN`) that wakes the watching session — no timer/poll. The all-green re-check means exactly one honest post per green PR (only the last-finishing run sees all-green). `continue-on-error`, never reddens CI, **non-blocking** (not a required context). Ported from the proof in sibling `or-agents` (live 2026-07-24). |
...
**Dismantle / utility workflows (kept until their job is done):** `gcp-action.yml` (risk-gated GCP command channel — `phase=propose` classifies green/yellow/red via `scripts/gcp-classify.sh`; red → Or's Telegram ✅ → the MCP dispatches `phase=execute`; this is how GCP projects are deleted), `propose-repo-delete.yml` + `.claude/commands/delete-repos.md` (Telegram-gated repo deletion — AI proposes, Or approves; `or-factory-master` hard-refused), `decommission-test-projects.yml` / `bulk-delete-repos.yml` (bulk GCP/repo cleanup), `publish-static-site.yml` (the "idea → live URL" publish engine, on the `dispatch_workflow` allowlist), `set-repo-visibility.yml` (manual `workflow_dispatch(repo, visibility)` — flips a repo private/public via the broker App's `administration:write`, WIF-only, token scoped to the target repo, verifies in-run; how a repo's visibility is changed without a manual click), `set-repo-automerge.yml` (manual `workflow_dispatch(repo)` — turns ON a repo's `allow_auto_merge` setting via the same broker-App/WIF pattern; reads the prior value, PATCHes it true, verifies in-run. This is what lets the operator's "Auto-merge when ready" toggle work — GitHub refuses to arm auto-merge on a repo whose `allow_auto_merge` is off. Idempotent; the durable fix for future siblings lives in `protect-system-main.yml`, this workflow flips an already-existing repo).

**Sibling-system builder (permanent):** `bootstrap-system-infra.yml` (manual `workflow_dispatch(system_name)` — the infra engine behind `/new-system`: as the broker via WIF it creates a new isolated GCP project + WIF + runtime/deploy SAs + copies generic secrets + mints the per-system OpenRouter key + MCP bearers + runtime shells + sets repo variables. Preflight refuses to run if the project already exists, so a stray dispatch is a safe no-op. Does NOT create the repo or push content — `/new-system` (`.claude/commands/new-system.md`) drives that from `templates/new-system/`). `protect-system-main.yml` (manual `workflow_dispatch(system_repo, required_contexts_json)` — the **hardening step** of `/new-system`, run after the system's foundation is merged + CI green: as the broker via WIF it mints an `administration:write` token scoped to the target repo and applies the `protect-main` ruleset via `scripts/ensure-protect-main-ruleset.sh` [PR-required, the 2 CI contexts required, no force-push/deletion], then enables `delete_branch_on_merge` **and** `allow_auto_merge` (the latter so a fresh sibling can use the operator's "Auto-merge when ready" toggle out of the box — GitHub refuses to arm auto-merge on a repo without it). `bootstrap-system-infra.yml` deliberately leaves main **unprotected** — this closes that gap. Reuses the same generic script as factory's own `protect-main.yml`; default contexts are the 2 a clean sibling inherits [`shellcheck + yamllint`, `Scan for committed secrets`]).

The gold template also ships `templates/new-system/.github/workflows/notify-routine-ci-green.yml` (the same CI-green → wake-the-watching-session bridge factory runs, see the Workflows table) — fully generic, no placeholder to fill, listening to a clean sibling's `Pipeline Tests` + `Secret Scan` — so every future `/new-system` sibling inherits it. It is non-blocking and deliberately **not** in `protect-system-main.yml`'s required set.

## MCP

The factory MCP server (`5b6e937f-c064-4cfd-88c4-ef93df38fa87`) provides **read-only inspection tools** (`verify_*_system` **plus** `verify_mcp_server`, `list_all_systems_inventory`, `inspect_*`, `tail_*_logs`, `get_billing_costs`, `gcp_project_quota_status`, `probe_endpoint`, `list_n8n_workflows`, `list_workflow_runs`, `get_run_jobs`, `read_github_actions_run_logs`, …), **org-wide GitHub read tools** (`get_repo`, `get_file_contents`, `search_code`, `list_pull_requests`/`get_pull_request`, `list_commits`, `list_repo_variables`/`get_repo_variable`, …), plus **three write-side tools** — **`dispatch_workflow`** (triggers the allowlisted workflows via the org-wide broker App — a sanctioned **cross-repo** write), **`emit_event`** (emits one observability event to Axiom/Telegram/Linear; writes only to those sinks, never to a repo), and **`append_agent_data`** (the second sanctioned **cross-repo** repo-write: appends one line to `agents/<agent>/data/<file>` on the unprotected `agent-data` branch of `edri2or/or-agents` — the broker token is down-scoped to that one repo + `contents:write` server-side, the path is composed under a `agents/<agent>/data/` allowlist server-side, and `main` stays locked; see `services/mcp-server/src/agent-data.ts`). Use the read tools to verify freely.
...
**or-factory-master has no n8n instance of its own.** It runs a Cloud Run gateway, not n8n — this repo defines **zero** n8n workflow JSON, and `/n8n/<system>/mcp` is a **proxy into *other* systems' n8n** (`https://n8n-<system>.or-infra.com`, the target system's key injected server-side). Verified 2026-07-18: at that time the only Railway n8n was **or-aios's** (`https://n8n-or-aios.or-infra.com`); as of 2026-07-19 the sibling system **or-agents** has its own live Railway n8n too (`https://n8n-or-agents.or-infra.com`). Either way **factory itself still has no n8n** — so **do not use any n8n connector "for factory."** (The `factory-master:` prefix on that instance's workflows is a brand stamp the now-deleted provisioning templates put on a *system's* workflows — it is not evidence of a factory-owned n8n.) The n8n connectors `n8n-live` / `N8N-or-aios` belong to **or-aios** (its `docs/session-mcp-map.md`); `n8n-all` / `n8n-or-edri-base` / `n8n-or-tok` belong to neither of these two repos.

**Connector ownership across Or's two systems (or-factory-master + or-aios)** — proven live 2026-07-18:
- **Shared (both systems):** `factory` (`20d5b7f9` — infra + both repos) and **GitHub** (the builtin `github` + the `GitHub CLAUDE` account connector `f492dc70` both authenticate as `edri2or-commits`; the builtin is Claude Code's, `GitHub CLAUDE` is Cowork's — **not** a removable duplicate, see the cross-surface note below).
...
- **Third sibling (`or-agents`, live 2026-07-19):** a separate deployed system with its OWN GCP project + Railway n8n (`n8n-or-agents.or-infra.com`) + `.mcp.json` (`factory` + `n8n-live` on `/n8n/or-agents/mcp`) + the shared Google route. It is **out of scope for a factory session** (factory uses `factory` + GitHub only, unchanged) — its connectors belong to an *or-agents* session, exactly as or-aios's belong to an or-aios session. The two-system map above is the factory↔or-aios picture; or-agents is a peer of or-aios, not of factory.

**Claude Code vs Cowork (factory session) — proven live 2026-07-18** (a Cowork session Or ran on this repo):
- **GitHub:** Claude Code → builtin `github`; Cowork → **`GitHub CLAUDE`** (`f492dc70`). Same identity `edri2or-commits`. Cowork has no builtin `github`, so `GitHub CLAUDE` is its **only** GitHub path — **keep it** (it is redundant only inside a Claude Code session).
- **Google:** both surfaces → `factory-master-actions-mcp` (`b6d78a00`) → 3 calendars, primary `edri2or@gmail.com`.
- **n8n:** none. Asking the `factory` connector for factory's own n8n (`list_n8n_workflows` with `systemName=factory`) fails with **`n8n_key_missing`** ("n8n-api-key not found in factory Secret Manager") — a **live** confirmation that factory has no n8n instance. The other n8n connectors (`N8N-or-aios` / `n8n-live`) are or-aios's and were correctly not touched.
...
- **`append_agent_data` (write) is NOT gated on claude.ai — proven live 2026-07-21.** Not every write tool trips the server-side gate; `dispatch_workflow` does, `append_agent_data` does not. A smoke call on Or's claude.ai wrote a real row to `agents/gmail/data/smoke.md` on the `agent-data` branch (`appended:true`, commit `94fe3d76`) with no approval prompt — so an or-agents agent on a managed surface (claude.ai/Cowork) can persist its own data-log line directly. (The path-allowlist also proved live: a `_smoke.md` attempt returned `invalid_path` — the tool ran, and the lock held.)
...
- **Google tools on the web — the `localhost:3002` OAuth-fallback trap is now closed at the gateway (live + proven 2026-07-21, PR #634).** The gateway forces the shared label onto every Workspace `tools/call` **server-side** (`services/mcp-server/src/workspace-mcp-proxy.ts` → `forceWorkspaceUserEmail`, fed by `WORKSPACE_GOOGLE_ACCOUNT_LABEL` emitted on **both** containers by `render-mcp-service-yaml.sh`), so a caller's **wrong or absent** `user_google_email` is silently corrected to `edriorp38@or-infra.com` (the storage-key label; the data is still Or's `edri2or@gmail.com`) instead of dropping into the dead `localhost:3002` consent link. Proven live: calls made with the deliberately wrong `edri2or@gmail.com` returned real Gmail/Calendar/Drive data in **both** a factory session and a Cowork session. The old failure mode (a wrong-email guess → `localhost:3002`, misread as "connector not authorized") **can no longer occur** — origin was the "2026-07-21 Google token down" false alarm, which was actually a Cowork wrong-`user_google_email` guess, not a fallen token. Still: `edriorp38@or-infra.com` stays the canonical label, and never tell Or to "add the connector again."
