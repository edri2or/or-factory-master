...
| `protect-main.yml` | `push: main` (self + `scripts/ensure-protect-main-ruleset.sh`) + manual | Idempotent: creates/updates the `protect-main` ruleset on `or-factory-master` via the broker App. Requires the 4 CI contexts (`Changelog gates` / `shellcheck + yamllint` / `Scan for committed secrets` / `Supply chain gates`), PR-required (0 approvals), no force-push/deletion. |
