...
> **הערה היסטורית (2026-07-18).** המסמך הזה נכתב עבור מכונת בניית-הסוכנים של המפעל, שפורקה בקיפול `or-factory-master` למערכת אחת שמשרתת את `or-aios`. התבנית `templates/agent-design-spec.md` שהוא מפנה אליה **כבר לא קיימת בריפו זה** (נשאר רק `templates/devplan/`). אזכורים אלה הם **רקע היסטורי — לא מצב חי**. הליבה החיה מתוארת ב-`CLAUDE.md`.
