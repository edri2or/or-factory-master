...
`db-setup` (יצירת טבלאות Postgres, כולל `agent_trace_events` — ראה `bot-trace-observability` ב-§6), משדרג את `unknown-agent` ל-`memoryPostgresChat`
...
שקורא ל-subworkflow `postgres-named-queries` עם whitelist קבועה של 8 שאילתות
SELECT (ללא SQL חופשי; ראה `bot-trace-observability` ב-§6), וגוש `SYSTEM-INFO` JSON שמוזרק ע"י `configure-agent-router.yml`
...
`triggerToStartFrom:{name:"Run Once"}`). **אזהרה (תוקן ב-`bot-trace-observability`):** ב-n8n 2.x
body של `{destinationNode}` בלבד (בלי `workflowData`/`triggerToStartFrom`) מריץ **רק** את צומת
ה-trigger ו-"Create Tables" לעולם לא רץ → אף טבלה לא נוצרת, אבל `/run` מחזיר 200 → "PASS" שקרי.
ה-configure שב-`bot-trace-observability` החזיר את ה-body המלא **+ אימות אמיתי** (קריאת
`lastNodeExecuted=="Create Tables"`, לא רק HTTP 200):
...
| **ראות מובנית על הבוט (claim-vs-actual)** | ✅ נשלח | פיתוח `bot-trace-observability` — כל מערכת חדשה נולדת עם **טבלת `agent_trace_events`** (זוג ניסיון→תוצאה per קריאת-כלי: `tool_name`, `status` [`attempted`/`success`/`failed`], `input_summary`/`output_summary` JSONB, `started_at`/`finished_at`/`duration_ms`, עוגן-תור in-SQL `max(id)` מ-`n8n_chat_histories` — בלי חיווט). אינסטרומנטציה בגבול-הכלי של 3 כלי-הקריאה (`postgres-named-queries`/`github-readonly`/`railway-readonly`): `Mint Trace`→`Write Trace Attempt`→…→`Write Trace Result`→`Return Tool Payload` (משדר מחדש את payload-הכלי כך שחוזה ה-LLM נשמר), הכול `onError:continueRegularOutput` (לעולם לא שובר תשובת-בוט); fallback אם אין PG (jq strip). 3 שאילתות חשיפה חדשות ב-`postgres_named_query`: `conversation_transcript` (תמלול, `COALESCE` על צורות LangChain), `tool_trace_recent`, `claim_actual_mismatch` (גלאי-הפער: `attempted` בלי `success` → `silently_dropped` — תבנית n8n #28843). חשוף לסוכן-פיתוח דרך `/mcp/system-tools`. **מאומת חי מקצה-לקצה על `factory-test-061`**: round-trip טלגרם → `tool_trace_recent` הראה 5 קריאות (3 כלים) עם ניסיון→תוצאה; `conversation_transcript` החזיר שיחה מלאה; `claim_actual_mismatch` תפס פער מאולץ. **האימות החי גם תפס+תיקן באג קריטי:** ב-2.x ה-/run של `db-setup` איבד את `workflowData` → אף טבלה לא נוצרה באף מערכת (false-positive של HTTP 200) — שוחזר ה-body המלא + אימות `lastNodeExecuted`. מערכות חדשות בלבד. |
| **תיעוד שליחת כפתורי-האישור (send→outcome)** | ✅ נשלח | פיתוח `button-send-outcome-trace` — מסלול שליחת-הכפתורים ב-`request-write-action` כבר לא מחזיר הודעת-הצלחה מקובעת. צומת-השליחה קיבל `onError:continueErrorOutput`, ואחריו `Guard message_id` (IF `notEmpty` על ביטוי-coalescing `$json.result?.message_id ?? $json.message_id` — עמיד לשני מבני-ההחזרה האפשריים של צומת-טלגרם) מסמן `delivered` רק עם הוכחה חיובית, ולא סומך על ניתוב-הענף לבד (n8n עלול לנתב כשל לענף-הצלחה). צמד `Mint Trace`→`Write Trace Attempt` (`tool_name='request_write_action_send'`, `status='attempted'`) לפני השליחה, ו-`Write Trace Result OK` (`status='success'` + `message_id` ב-`output_summary`) / `Write Trace Result Failed` (`status='failed'` + השגיאה, דרך `Normalize Failure` שמ-`esc()` את הסיבה משני המקורות — פורט-השגיאה והגארד-false) אחריה; כל צמתי-ה-trace `onError:continueRegularOutput`+`alwaysOutputData:true`. ה-`Tool Response` הסטטי הוחלף ב-`Tool Response Sent` (`status:'sent'`, כולל ה-`message_id`) / `Tool Response Failed` (`status:'send_failed'`, הודעה כנה). reuse של עמודת `output_summary` JSONB הקיימת (אין שינוי-סכמה, `db-setup` לא נגע); נחשף לסוכן דרך `claim_actual_mismatch`/`tool_trace_recent`. **מגבלה מובנית (Telegram desktop #29497):** גם 200 OK + `message_id` תקין אינם מבטיחים שהכפתורים *רונדרו* בלקוח — קיים, נדיר, ו**לא ניתן-להוכחה ע"י trace** (ה-`message_id` הוא תקרת-ההוכחה; אין API למצב-מסירה). **פער ידוע — "סוג-ב'", מחוץ-לסקופ בהחלטת Or:** אם ה-LLM *טוען* בתשובתו ששלח כפתורים בלי לקרוא לכלי כלל — אין פעולה לתעד; זו שכבת-פרומפט (הוראת-סוכן), לא נסגרה כאן. **הוכחה:** סטטית (טענות-גרף ב-jq + `node --check` על ה-Code-nodes + שערי-CI) + **Day-0 חיה** — ייבוא+פרסום נקי של הגרף החדש ל-n8n 2.25.7 חי על `factory-test-398` דרך `prove-on-test-system.yml` (+ מעבר ה-CI של המערכת עצמה). round-trip התנהגותי מלא לא בוצע (מגבלת-כלים בסשן + בוט-טסט חולק טוקן-טלגרם); הדפוס זהה ל-`bot-trace-observability` (#382) שהוכח חי מקצה-לקצה. מערכות חדשות בלבד. |
