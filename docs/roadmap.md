...
- Railway project named after `vars.SYSTEM_NAME`, Postgres service via `ghcr.io/railwayapp-templates/postgres-ssl:17` with a persistent volume mounted at `/var/lib/postgresql/data`, and an n8n service from `n8nio/n8n:2.25.7` (1.85.4 originally; bumped to 1.121.0 to patch CVE-2026-21858 "Ni8mare" unauthenticated-RCE; then 1.121.0 → 2.25.7 in `n8n-2x-upgrade`, which also added an in-place image-upsert upgrade path for existing systems)
...
**Follow-up shipped — web search & research (`research-web-search`).** The bot can now search the live internet. Both the `research-agent` (converted to an `agent` node on `claude-sonnet-4.5`) and the general chat `unknown-agent` carry two Tavily HTTP tools — `web_search_quick` (fast, `search_depth=basic`) and `web_search_extended` (deeper, `advanced`) — created by `configure-agent-router.yml` from the `tavily-api-key` generic secret (soft-fail: stripped if the key is absent). Sources come back in a `[[SOURCES]]` block that "Egress Validation" exempts from URL redaction (full links, deduped, ≤10); the rest of the reply stays allowlist-redacted. Prompts are hardened (never expose tool names, never claim it can't browse, stick to results). A supporting factory workflow — **`refresh-system-agents.yml`** — applies such agent-workflow fixes to an existing live system *without* a re-provision (the cheap iterate-on-one-live-system loop). Verified live on `factory-test-tavily2`. True minutes-long **async deep research** was deferred on n8n 1.121 (its Wait-in-subworkflow is unstable, and a synchronous minutes-long search would hit the Telegram webhook timeout). **Unblocked by `n8n-2x-upgrade` (done).**

**Follow-up shipped — async deep research (`async-deep-research`).** An explicit trigger phrase ("תחקור לעומק" / "מחקר עמוק" / "deep research") now kicks a **background** multi-search investigation: the router's new `Detect Deep Research` → `Kick Deep Research` fires the `deep-research` sub-workflow **fire-and-forget** (`executeWorkflow` with `waitForSubWorkflow:false`) and returns an **instant ack** on the synchronous line, dodging the Telegram webhook timeout. The `deep-research` workflow (Claude `sonnet-4.5`, `maxIterations:20`, the two existing Tavily tools) runs for minutes, then **self-sends** the finished report to Telegram (its own node, like `tg-proactive`) — egress-validated (same exec/eval+`<script>`+allowlist gate as the router) and chunked to ≤3500-char Telegram messages with the `[[SOURCES]]` block always delivered last. No new secret/provider/cost (reuses Tavily + OpenRouter + Telegram). Graceful: the whole deep path is stripped from the router when Tavily/Telegram/chat-id are absent. **Proven live end-to-end on a fresh n8n 2.25.7 system** (`factory-test-053`): instant ack → independent background execution → full Hebrew report in 4 messages with 10 real sources. The live test also caught + fixed a real Day-0 bug — on n8n 2.x, `tg-inbound` could not publish while its `executeWorkflow` sub-deps (`tg-vision`/`tg-voice-stt`/`pending-actions-executor`) were unpublished, so those are now published too. New systems only.
...
| 6 | Move the shared Google OAuth redirect URI off or-adhd-agent to a stable address (a Google-console config, independent of the GitHub repo). **Update 2026-07-17:** the `or-adhd-agent` **GitHub repo was deleted** in the archived-repo cleanup, *ahead* of this item. That was safe for the shared identity — it lives in GCP SM (`factory-test-7`, still ACTIVE) + the gateway, **not** in the repo — and was re-proven healthy right after (`google-mcp-smoke` run 29578335037, live read PASS). This item **stays open**: the redirect URI still points at the dead `n8n-or-adhd-agent` domain and wants cleanup. | mcp-birth-bundle | Open (queued) |
...
Operational (not a dev, approval-gated): the legacy non-factory **GitHub repos** — or-tok,
tokile, or-edri-2, and or-adhd-agent — were **deleted 2026-07-17** via `propose-repo-delete.yml`
+ Or's Telegram ✅ (part of an archived-repo cleanup; verified by `list_repos`). Still pending
(each through the Telegram-✅ gates, never automatically): any leftover Railway/GCP resources for
these + `project-life-130`, and follow-up 6 above (the shared redirect URI). **Do not** tear down
GCP `factory-test-7` — it holds the shared `gmail-oauth-*` secrets (the live Google identity).
