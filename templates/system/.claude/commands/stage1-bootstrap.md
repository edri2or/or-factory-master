...
## Org constants (hardcoded — replace for your org)

| Constant | Value |
|----------|-------|
| `DOMAIN` | `[your-domain]` |
| Seed WIF | `projects/[your-gcp-project-number]/locations/global/workloadIdentityPools/github-pool/providers/github-provider` |

⚠ If target org is not `[your-org]` / `[your-domain]`, replace these values and confirm with user.
...
| `PAT_SECRETS_WRITE` GitHub Secret | Must exist — used by `gcp-bootstrap.yml` to set GitHub Secrets (contains `GITHUB_PAT_SECRETS_WRITE` value) |
...
⚠ **Known failure:** `roles/secretmanager.secretCreator` does **not** exist as a
    ...
    steps:
      ...
      - name: Bootstrap Railway project (idempotent)
        ...
        run: |
          ...
          # Use workspace-scoped query — { projects } returns personal workspace only
      ...
      - name: Store Railway IDs as GitHub Secrets
        env:
          GH_TOKEN: ${{ steps.secrets.outputs.GITHUB_PAT_SECRETS_WRITE }}   # PUSH_TARGET_TOKEN deleted
      ...
      - name: Create Cloudflare DNS record (idempotent)
        ...
        run: |
          EXISTING=$(curl -sf \
            "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?name=<project-name>.[your-domain]&type=CNAME" \
      ...
      - name: E2E — Verify Railway project via API
        env:
          ...
          PROJECT_ID: ${{ steps.railway.outputs.project_id }}
        run: |
          # Declare variable type explicitly — Railway enforces strict GraphQL validation
      ...
      - name: E2E — Check DNS resolves
        run: |
          HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 15 \
            https://<project-name>.[your-domain] || echo "000")
        ...
        run: |
          {
            ...
            echo "| Cloudflare CNAME \`<project-name>.[your-domain] → railway.app\` | ✅ |"
...
fetched from the Secrets Broker (`PUSH_TARGET_TOKEN` is **retired**):
```bash
...
| Cloudflare CNAME `<project-name>.[your-domain] → railway.app` | ✅ |
...
|---------|---------|-----------|-----|
| Duplicate [your-railway] projects | Multiple projects same name | `{ projects }` returns personal workspace only | Use `{ me { workspaces { projects } } }` — already in template |
| GraphQL validation error | `Variable "$id" is not defined` | Missing type annotation | `query($id: String!)` — already in template |
| PERMISSION_DENIED on token write | `gcloud secrets versions add` fails | SA missing `secretVersionAdder` on secret | Add `version_adder` resource to `secrets.tf`, re-run `gcp-bootstrap.yml` |
| Terraform Error 409 | `Secret X already exists` | Secret created outside Terraform | Add `import {}` block to `secrets.tf` |
| Terraform Error 400 | `secretCreator not supported` | Role doesn't exist at project level | Remove project-level IAM binding, use resource-level only |
...
**Agent behaviour:**
Reads git remote → derives project name. Reads `terraform/secrets.tf` — finds
