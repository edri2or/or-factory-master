...
name: code-agent
description: >-
  The code specialist — handles code / repo / GitHub-shaped requests and returns {reply} to the orchestrator. Load this skill to understand, run, develop, or reason about THIS
  system's "code-agent" n8n workflow — the code specialist the Agent Router dispatches for code / repo requests. It is a map, not a manual: it routes
  you to the live workflow via the n8n-live and factory MCP servers (.mcp.json)
  and AGENTS.md, never to hard-coded ids or secrets. Triggers on code-agent, code, github, repo, development.
---

# code-agent

This skill describes the **`code-agent`** n8n workflow shipped into this system by
...
The code specialist — handles code / repo / GitHub-shaped requests and returns {reply} to the orchestrator. Trigger: invoked by the Agent Router (returns the {reply} contract).

## Read its live state first (read-only)

- **`factory` MCP** (`.mcp.json`) — `list_n8n_workflows`, `inspect_n8n_execution`,
  and this system's `/mcp/system-tools` named queries — see how `code-agent` actually ran
...
- **`n8n-live` MCP** (`.mcp.json`) — run and develop `code-agent` against this system's
