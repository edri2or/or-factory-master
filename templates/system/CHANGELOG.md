...
| - | feat | The general chat agent (`unknown-agent`) also carries `github_readonly` (incl. `read_file`) and `railway_readonly`, so the bot answers GitHub / file / Railway questions in any phrasing — not only when a message is classified as an operational task. Read-only. See `changelog.d/` for detail. |
