...
| 5 | אימות פוסט-תיקון + סגירת תיק | completed | `.github/workflows/oil-autofix-verify.yml`, `scripts/oil-verify.sh`, `pipeline-tests.yml` |
| 6 | תיעוד (חריג מכוון ותחום) | completed | `docs/oil-autofix.md`, `CLAUDE.md`, `docs/roadmap.md` |
...
**אקטיבציה אומתה טכנית + אימות חי בביצוע:** ה-MCP נפרס מחדש (אוטומטית עם מיזוג ה-allowlist),
וכל הסודות האמיתיים נטענו. **הוכח שהגשר ער:** `POST /telegram-webhook` בלי secret_token החזיר
**401** (ולא 503) — הסוד נטען. עכשיו רץ האימות החי המלא (אופציה 2, מה ש-DEVPLAN דורש): fixture
באג אינרטי `scripts/oil-selftest-draftpr.sh` חוזר ל-main, ואז תיק Linear זמני ידליק את הלולאה
המלאה → הודעת טלגרם ל-Or → ✅ → מיזוג ע"י `oil-autofix-approver`. ניקוי אחרי.

**האימות חשף מתח אמיתי + תוקן:** ריצת OIL-20 — החוקר→מתקן רצו, אבל המתקן הוסיף `CHANGELOG.md`
(כדי לרצות את שער ה-CI) → 3 קבצים → שער-הבטיחות (`MAX_FILES=2`) הסלים *לפני* שהגשר נגע. זה
מתח-תכנון אמיתי (כל תיקון צריך CHANGELOG, אבל השער מגביל ל-2). **תוקן:** ה-prompt של המתקן אוסר
עכשיו לגעת ב-`CHANGELOG.md`, וה-workflow עצמו מוסיף את רשומת ה-CHANGELOG בשלב פתיחת-ה-PR
(דטרמיניסטית, אחרי שהשער אישר את 2 הקבצים של ה-AI). נותר: להריץ שוב את OIL-20 → הפעם תיפתח PR
טיוטה → הודעת טלגרם → ✅ → מיזוג.

**ריצה 2 (OIL-20) — התקדמה ותפסה באג שלי:** הפעם המתקן נשאר ב-2 קבצים, **השער עבר**, ושלב
פתיחת-ה-PR התחיל — אבל ה-commit של ה-CHANGELOG ב-workflow היה עם סדר-ארגומנטים שגוי
(`git commit -- CHANGELOG.md -m "…"` → git פירש את `-m` כשם-קובץ), ה-commit נכשל, והשלב יצא לפני
`opened=true` → שלב הטלגרם דילג. ללא ענף/PR יתום (הכשל לפני ה-push). **תוקן:** `-m "…"` לפני
`-- CHANGELOG.md`. נותר: ריצה 3 של OIL-20 → אמורה להגיע לטלגרם.

**3 ריצות חי + מעבר לאימות דטרמיניסטי:** ריצה 3 — החוקר קרא את ה-DEVPLAN וה-fixture, הבין נכון
שזה self-test מתוכנן, וסיווג `transient-infra` (לא תיקן). כלומר ה-AI הוא חוליה לא-דטרמיניסטית
(3 ריצות, 3 תוצאות שונות). הגשר עצמו מוכח ער (`/telegram-webhook`→401), אבל `/oil-approval-register`
חסום-admin ולא נגיש מהסוכן — אין דרך דטרמיניסטית לירות את חוליית ה-✅→מיזוג דרך ה-AI. **הפתרון:**
נוסף מצב `mode=smoke` זמני ל-`oil-autofix-investigate.yml` — job נפרד (רק ב-workflow_dispatch,
mode=smoke) שבלי AI פותח PR אינרטי בזהות הברוקר וקורא ל-`/oil-approval-register` בדיוק כמו
הנתיב האמיתי → הודעת טלגרם ל-Or → ✅ → מיזוג בזהות `oil-autofix-approver`. מבודד ומוכיח את הגשר
דטרמיניסטית. פיגום זמני — יוסר בניקוי שלב 4 יחד עם ה-fixture.

**ה-smoke חשף באג סמוי שלישי (וקריטי):** הריצה הראשונה של `mode=smoke` נכשלה ב-`git push` עם
**403 "Write access not granted"**. השורש: `actions/checkout` (ברירת-מחדל persist-credentials)
מגדיר `http.extraheader` עם ה-GITHUB_TOKEN של ה-workflow (כאן contents:read בלבד), וזה **דורס**
את טוקן-הברוקר ב-URL של ה-push. זה באג **משותף לנתיב הייצור** — הוא לא נתפס קודם כי ריצה 2 מתה
לפני ה-push. **תוקן:** שני שלבי ה-checkout (investigate + smoke) עם `persist-credentials: false`.
נותר: ריצה חוזרת של `mode=smoke` → הודעת טלגרם → ✅ → מיזוג.

**הגשר הוכח כמעט-במלואו (באג 4+5 חי):** ריצת smoke 2 — ה-push הצליח, PR אינרטי #188 נפתח,
`/oil-approval-register`→200, **הודעת טלגרם הגיעה ל-Or עם כפתורים, Or לחץ ✅**, האימות (secret_token
+ from.id) עבר — אבל **המיזוג עצמו נכשל** (`merge_failed` ×2). שני באגים: (א) הקוד הסיר draft ב-REST
PATCH (GitHub מתעלם) → ה-PR נשאר draft → לא ניתן למזג; (ב) branch protection דורש 4 בדיקות ירוקות
ו-`mergeable_state` היה unstable. + באג-תזמון: ה-callback נענה אחרי המיזוג → טלגרם עשה retry →
merge_failed כפול. **תוקן:** (א) הסרת draft דרך GraphQL `markPullRequestReadyForReview`; (ב)
...
**הערת התקדמות אחרונה:** PR-1 (בניית המאמת) נכתב. נוסף `oil-autofix-verify.yml` — workflow
פוסט-מיזוג שמופעל ב-`push: main`, מזהה מיזוג OIL לפי כותרת ה-squash (`oil-autofix: OIL-NN — … (#PR)`),
משחזר את בדיקת-השחזור מתוך ה-commit הממוזג (הקובץ שנוסף תחת `scripts/tests/`), ומריץ אותה דרך
`scripts/oil-verify.sh` (שער טהור: whitelist `bash <file>`, `env -i` מנוקה, רק אחרי שלילת ה-creds).
הצלחה → `issueUpdate` למצב completed + comment סוגר + הודעת טלגרם "✅ תוקן ואומת"; כשל → comment +
טלגרם "🚨 נכשל באימות", **בלי לסגור**; ללא-reproducer → הסלמה רכה (comment, בלי אזעקת-שווא). בדיקה-
עצמית `scripts/tests/oil-verify-selftest.sh` (5 מקרים: verified/failed/malformed/missing/empty) חוברה
ל-`pipeline-tests.yml`, ו-`oil-verify-failmode.sh` הוא fixture לבדיקת נתיב-הכישלון החי דרך `workflow_dispatch`.
אומת מקומית: shellcheck (severity=error) נקי, yamllint נקי, כל שערי ה-supply-chain עוברים, הבדיקה-העצמית
ירוקה. **דורמנטי** — לא עושה כלום עד שתיקון OIL אמיתי יתמזג. **PR-1 מוזג** (#203, squash) ואומת חי
שהמאמת ישן (רץ על המיזוג של עצמו ודילג, `conclusion: skipped`; 4 בדיקות main ירוקות). **PR-2** — ריצת
אימות חיה מבוקרת על תיק OIL-22: ממזג fixture-מעבר (`scripts/tests/oil-verify-passmode.sh`) בכותרת
`oil-autofix:` כדי להעיר את המאמת חי. **אומת חי (2026-05-29):** PR-2 (#204) מוזג → המאמת רץ
(`conclusion: success`), שִׁחזר את ה-fixture, הריץ על main → עבר, **סגר את OIL-22 ל-Done** + הערה סוגרת
+ הודעת טלגרם "✅ תוקן ואומת". **נתיב-הכישלון (#205) חשף באג אמיתי:** GitHub מריץ שלבי `run:`
תחת `bash -e`, וה-capture `out=$(bash oil-verify.sh …); rc=$?` בשלב ה-verify **הפיל את השלב מיד** כש-
ה-reproducer יצא בכישלון — `outcome=failed` לא נכתב, ולכן ה-comment + טלגרם "🚨 נכשל" דילגו בשקט (נתיב
ההצלחה לא הושפע — לכן OIL-22 עבד). **תוקן (PR-4):** `set +e` בשלב ה-verify (שמטפל בקודי-יציאה בעצמו) +
הקשחת `&&` ב-prep; הודגם-מחדש חי על OIL-23 עם `oil-stage5-failsmoke-2.sh`. **✅ שלב 5 הושלם
ואומת חי מקצה-לקצה (2026-05-29):** שני הנתיבים הוכחו — הצלחה (OIL-22 נסגר ל-Done לבד + "✅ תוקן ואומת")
וכישלון (OIL-23 נשאר פתוח + "🚨 נכשל באימות"). ה-fixtures הזמניים (`oil-stage5-failsmoke*.sh`) נוקו,
ותיקי-הבדיקה OIL-22/OIL-23 בוטלו.

**שינוי תוכנית:** **(א)** הפיצול המקורי לשני workflows (dispatcher ב-`pull_request: closed` → worker
ב-`repository_dispatch`) **בוטל לטובת workflow יחיד ב-`push: main`.** הסיבה: לפי הנחיית Or לא להישען על
ה-`GITHUB_TOKEN` המובנה — וה-dispatcher היה היחיד שנזקק לו (workflow מסוג `pull_request` אסור לו להחזיק
`id-token`, כך ששער ה-supply-chain `check-no-privileged-pr-workflows.sh` חוסם WIF שם). workflow מסוג
`push: main` *כן* רשאי להחזיק `id-token`, ולכן הוא ניגש ל-WIF→Secret Manager ישירות כמו שאר הפקטורי.
זיהוי מיזוג-OIL נעשה לפי כותרת ה-squash. **(ב)** פקודת בדיקת-השחזור משוחזרת **ללא GitHub token** מתוך
ה-diff של ה-commit הממוזג (הקובץ שנוסף תחת `scripts/tests/`), במקום מ-marker ב-גוף ה-PR שנשלף ב-API — לכן
`oil-autofix-investigate.yml` **לא** משתנה. נטו: אפס שימוש ב-GITHUB_TOKEN, פחות קבצים, ואותה הפרדת-זהויות.
...
**הערת התקדמות אחרונה:** הושלם. נוסף `docs/oil-autofix.md` (תיעוד מלא: זרימה מקצה-לקצה, רכיבים,
מודל-אבטחה, הפעלה/בדיקה, מגבלות v1). ב-`CLAUDE.md` נוספו שתי שורות ל-Workflows table
(`oil-autofix-investigate.yml` + `oil-autofix-verify.yml`) + הערה אחרי "The one rule" ש-OIL הוא
החריג היחיד המאושר ל-"no auto-chain / no issue-based reporting" (מאומת בכל צעד, מבוסס-Linear, אישור-
אנושי בטלגרם). ב-`docs/roadmap.md` נוסף "Phase G — OIL auto-fix loop (done)" וסומנו שני הסעיפים
ב-"Things we are deliberately not building". שינוי markdown-בלבד (אין קבצי-קוד → שערי ה-CI עוברים).
**הפיתוח נסגר:** front-matter `status: completed` (משחרר את שער ה-devplan; שלב 7 דחוי לפיתוח `/dev-stage` נפרד).
...
- שלב 5 הושלם — עכשיו אחרי שתיקון נכנס, המערכת **בודקת לבד שהוא באמת עבד** (מריצה מחדש את בדיקת-השחזור על הקוד הממוזג) ורק אז **סוגרת את התיק** ושולחת לך "✅ תוקן ואומת". אם האימות נכשל — היא עוצרת, משאירה את התיק פתוח, ושולחת "🚨 נכשל באימות". אימתנו את שני המצבים חי (תיק-בדיקה נסגר לבד; תיק-בדיקה אחר נשאר פתוח עם התראה). בדרך אפילו תפסנו ותיקנו באג אמיתי — בזכות זה שבדקנו חי. הלולאה האוטונומית מקצה-לקצה סגורה.
- שלב 6 הושלם — תיעדנו את כל הלולאה במסמך אחד ברור (`docs/oil-autofix.md`), וסימנו ב-CLAUDE.md וב-roadmap שזו "יוצאת-דופן מבוקרת" לכללי הפקטורי (כי כאן הכול מאומת ועם אישור שלך בטלגרם). **בזה הפיתוח נסגר** — לולאת התיקון האוטונומית לתקלות חיה ומלאה. (שלב 7 — הרחבת סוגי-הקוד שהמערכת יודעת לתקן — דחוי בכוונה לפיתוח נפרד בעתיד.)
...
- 2026-05-29 · **Stage 135** — זהות סוכן לכל מערכת ב-`AGENTS.md` (כרטיס מפעיל קבוע + שדה `system_purpose` + שם סוכן אוטומטי). שינוי פקטורי עצמאי; הפירוט המלא ב-`CHANGELOG.md`.
