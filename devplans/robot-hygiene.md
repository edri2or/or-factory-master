...
|---|---|---|---|
| 1 | פטור ל-`oil-autofix/*` בשער-הפיתוח (#9) | pending | `scripts/check-devplan-updated.sh`, `scripts/tests/check-devplan-updated.bats` |
| 2 | תפיסת `gcloud gcloud` כפול בשער-הסיכונים (#8) | pending | `scripts/gcp-classify.sh`, `scripts/test-gcp-classify.sh`, `tests/gcp-classify-fixtures.yml` |
...
**Acceptance:**
- [ ] `check-devplan-updated.sh` מדלג (exit 0) כשהענף הוא `oil-autofix/*` (קורא
      `GITHUB_HEAD_REF` ואז `GITHUB_REF_NAME`), בלי לשנות שום workflow.
- [ ] בדיקת bats חדשה מוכיחה: תוכנית פעילה + שינוי קוד + בלי עדכון תוכנית, אבל ענף
      `oil-autofix/*` → PASS. ה-setup מנקה את משתני־הסביבה (hermetic).
- [ ] כל הבדיקות הקיימות ב-`check-devplan-updated.bats` עדיין עוברות.
...
**Acceptance:**
- [ ] `gcp-classify.sh` יוצא non-zero (exit 3) עם הודעת שגיאה ברורה כשהטוקן הראשון הוא
      `gcloud` (תופס גם `gcloud gcloud ...` וגם `gcloud` יחיד — שניהם פסולים לפי החוזה).
- [ ] `test-gcp-classify.sh` תומך בציפיית `reject` (יציאה non-zero) ב-fixture.
- [ ] שתי שורות fixture חדשות (`gcloud gcloud ...`, `gcloud secrets list`) → `reject`.
- [ ] אין רגרסיה: כל ה-fixtures הירוקים/צהובים/אדומים הקיימים נשארים כשהיו.
