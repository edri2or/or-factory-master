...
|---|---|---|---|
| 1 | DEPLOY_NONCE מאלץ revision חדש בכל פריסה → טוקן מחודש נטען | completed | `scripts/render-mcp-service-yaml.sh`, `.github/workflows/deploy-mcp-server.yml` |
...
**הערת התקדמות אחרונה:** הושלם ואומת חי. מוזג ב-PR #496; ה-deploy שלאחריו נוצר revision חדש, ה-`google-mcp-smoke` עבר, ו-Or אישר שכלי ה-Drive עובד מקצה-לקצה ב-claude.ai (קובץ נוצר ונקרא חזרה אות-באות).
...
- 2026-06-16: ✅ הושלם. התיקון מוזג, ה-Drive חזר לעבוד (אומת על ידך). הפיתוח נסגר.
