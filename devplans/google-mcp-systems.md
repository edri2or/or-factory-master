...
**0b פריסה + תיקון scope (2026-06-09):** הקוד מוזג (#350) ונפרס. הסמוק יצא **3/4** — כל
ה-plumbing הוכח (bearer → handshake → 11 כלי גוגל דרך ה-gateway). הקריאה הרביעית נכשלה כי
הטוקן המשותף הוא **כתיבה** (gmail.modify + calendar.events), ומצב `--read-only` דורש
`calendar.readonly` שאין לטוקן. **Or בחר: full mode (קריאה+כתיבה), כתיבה ב-HITL בשלב 1.**
תיקון ב-#351: `WORKSPACE_MCP_READ_ONLY=0`, scope בקרדנציאל = בדיוק 4 ה-scopes שהוסכמו,
והסמוק קורא `list_gmail_labels` (מכוסה ב-gmail.modify) במקום `list_calendars`. אחרי מיזוג
#351 → פריסה מחדש → סמוק צפוי 4/4. follow-up: טוקן read-only ייעודי יחזיר מצב קריאה-בלבד.
