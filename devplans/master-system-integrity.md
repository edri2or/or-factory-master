...
| # | שלב | סטטוס |
|---|---|---|
| 1 | מטריצת ההטמעה + אימות חי על or-edri-4 (קריאה בלבד) | in-progress |
| 2 | לולאת תיקונים על or-edri-4 (PRs קטנים, golden-gated, מוכח חי) | pending |
...
- **B1** db-setup: צומת `Verify Tables` (count) + `_db_ok` משוכתב (סינון workflowId בצד-לקוח, polling) + exit 1 על כשל-אמת.
