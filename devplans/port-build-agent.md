...
| 2 | שער "קול-אחד" + חיווט CI | pending | scripts/check-agent-single-voice.sh, templates/system/.github/workflows/pipeline-tests.yml, .github/workflows/* |
| 3 | ה-skill build-agent + שילוח חומרי-ייחוס למערכות | pending | .claude/commands/build-agent.md, templates/system/templates/agent-design-spec.md, templates/system/docs/* |
| 4 | תפשטות + שערים + PR | pending | templates/system/.claude/commands/build-agent.md (mirror), tests/golden/system/, changelog.d/, devplan |
...
**הוכחה תפקודית (באותו שלב):** השער מחזיר PASS על 5 סוכני-הפקטורי; PASS על סוכן-דוגמה
שנולד מ-`subagent.template.json`; FAIL על עותק עם node טלגרם מוזרק. אצפה: אריץ את שלושת
המקרים ואראה PASS/PASS/FAIL.
...
**הוכחה תפקודית (באותו שלב):** כל נתיב שמוזכר ב-build-agent.md קיים (אין הפניות-רפאים) —
אצליב ב-grep מול הקבצים בפועל; וצעדי-הרישום תואמים ללולאה הקשיחה האמיתית. אצפה: אריץ
הצלבת-נתיבים ואראה 0 חוסרים.
...
**הוכחה תפקודית (באותו שלב):** מטאטא-שערים מקומי ירוק (skills-mirror / system-golden /
golden-sync / check-agent-single-voice / devplan / changelog) ואז CI ירוק על ה-PR.
אצפה: אריץ את כולם ואראה rc=0, ואז אקרא את check-runs של ה-PR.
