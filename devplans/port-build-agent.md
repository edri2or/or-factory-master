...
- שלב 3 הושלם — כתבנו את המתכון עצמו (`build-agent`), משודרג: עכשיו הוא מחייב להוכיח כל
  ...
  לקובץ שלא קיים.
- שלב 4 הושלם — סגרנו: ה-CI על ה-PR ירוק לגמרי, והפיתוח נחתם. מעכשיו כל מערכת חדשה
  נולדת עם היכולת לבנות סוכנים נכון (מלמטה-למעלה, הוכחה לכל חלק). נשאר רק שתאשר ותמזג.
