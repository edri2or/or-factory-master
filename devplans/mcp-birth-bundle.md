...
| 3 | `.mcp.json.template` — סשני קלוד נולדים מחוברים | in-progress | `templates/system/.mcp.json.template`, `templates/system/AGENTS.md.template`, `.github/workflows/provision-system.yml`, `tests/golden/system/` |
...
**Acceptance:**
- [ ] `validate-templates.sh` + golden ירוקים; אין placeholder שיורי בקובץ המרונדר.
- [ ] `.mcp.json` קיים בריפו של מערכת-הטסט אחרי re-provision והתוכן זהה-בייט לצפוי.
...
**הערת התקדמות אחרונה:** קוד השלב כתוב: `.mcp.json.template` חדש (רק `${SYSTEM_NAME}`,
בלי שינוי allow-list), פסקת "Built-in MCP connections" ב-AGENTS.md.template, וצעד ה-docs
ב-provision מרנדר+דוחף גם `.mcp.json`. ‏golden רוענן (119 קבצים), validate-templates ‏3/3.
ממתין: מיזוג → provision של factory-test-047 → אימות קובץ + ‏401.
...
- **factory-test-046** — חיה (הוקמה 2026-06-10, reuse mode על `factory-test-25`, ‏0 מכסה).
  רכיבים: ריפו `edri2or/factory-test-046`, פרויקט Railway `d4ba2820-a7cc-462a-a2dc-83a86a3cb24a`
  (postgres+n8n, בלי Caddy — מדולג בתכנון ל-factory-test), ‏DNS ‏`n8n-factory-test-046.or-infra.com`,
  סודות ב-SM של `factory-test-25` (יידרסו בפרוביז'ן הטסט הבא). משמשת את שלבים 3+5
  (re-provision + mcpTrigger). פירוק: `decommission-test-system.yml` — רק בהוראת Or, בסוף הפיתוח.
