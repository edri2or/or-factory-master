...
(הגרסה הישנה נשמרה כ-rollback). ‏PR-B נכתב: ‏scopes env-driven ב-entrypoint
(‏WORKSPACE_MCP_SCOPES, ברירת-מחדל הסט-של-6), ‏tools=calendar gmail drive docs ב-render,
‏smoke מורחב 4→6 (כולל search_drive_files אמיתי), שורת Drive/Docs ב-systemMessage של
ops-agent + golden. ממתין: מיזוג → redeploy אוטומטי → smoke ‏6/6 → סבב Drive חי על 047.
