...
| 2 | תבנית: `factory_tools` על ops-agent + הקמת מערכת-טסט חיה | in-progress | `.github/workflows/provision-system.yml`, `templates/system/.github/workflows/configure-agent-router.yml`, `templates/system/workflows/n8n/ops-agent.json`, `tests/golden/system/` |
...
**Acceptance:**
- [ ] שערים סטטיים ירוקים (golden + changelog + devplan + shellcheck/yamllint).
- [ ] סבב חי: שאלה לסוכן שרק `factory_tools` עונה עליה (היסטוריית 5 דיפלויים אחרונים
      ב-Railway — ל-railway_readonly אין op כזה) חוזרת עם נתונים אמיתיים.
- [ ] בדיקת שלילה חיה: בקשת probe למערכת אחרת משתקפת כ-`tenant_blocked`.
...
**הערת התקדמות אחרונה:** כל קוד השלב כתוב (נוד `factory_tools` + פסקת systemMessage
ב-ops-agent; credential "Factory MCP" + sed + strip ב-configure; shell + mint תאום
ב-provision; golden רוענן). ממתין: שערים סטטיים → מיזוג → הקמת מערכת-הטסט מ-main
(באישור Or שניתן) → סבב חי + בדיקת שלילה.
...
- (טרם הוקמה מערכת-טסט בפיתוח הזה.)
