...
**Acceptance:**
- [x] google-mcp-smoke מורחב ירוק: רגרסיית gmail+calendar + כלי Drive וכלי Docs ברשימה
      + קריאת `search_drive_files` אמיתית (לא הודעת auth).
- [x] הסוכן במערכת-הטסט עונה על שאלת Drive אמיתית.
...
**הערת התקדמות אחרונה:** צעד-הקדם הושלם + ‏PR-A כתוב. חילוץ ה-scopes אומת מקוד החבילה
המוצמדת (wheel ‏1.21.1, ‏`auth/scopes.py`): ‏`SCOPE_HIERARCHY` מאשר ש-`auth/drive` מכסה
drive.readonly+drive.file ו-`auth/documents` מכסה documents.readonly — הסט המינימלי
המדויק = ‏6 (ה-4 הקיימים + drive + documents). ‏PR-A: ‏`rotate` ב-copy (עם no-op כשהערך
זהה), ‏workflow חד-פעמי `request-workspace-scopes-consent.yml` (מעדכן scope על credential
ה-n8n של or-adhd-agent בלי oauthTokenData → קישור consent ל-Or דרך בוט הפקטורי; תופעת
...
**Or לחץ Allow ✅ (11:2x).** גשר שנוסף: `rotate-shared-gmail-token.yml` — אין מסלול
אינטראקטיבי לשיגור workflow על ריפו-מערכת (ה-GitHub MCP של הסשן מוגבל לפקטורי; allowlist
ה-MCP לא כולל בכוונה workflows אד-הוק), אז workflow צר וקשיח (ריפו/קובץ/קלט קבועים, token
ברוקר בהיקף ריפו-אחד+actions:write) משגר את ה-extract על or-adhd-agent ומלווה עד סיום.
השרשרת נשארת agent-driven עם אימות בין צעדים.
**הרוטציה הושלמה (11:47):** גשר run 27273755388 ירוק (extract run 27273772103 על
or-adhd-agent הצליח) → copy run 27274019448: `PASS: ROTATED gmail-oauth-refresh-token`
(הגרסה הישנה נשמרה כ-rollback). ‏PR-B ‏(#365) מוזג → redeploy אוטומטי (run 27274453588).
**חסם אחרון שהתגלה חי:** ‏smoke ‏5/6 — ‏`Google Drive API is not enabled for your project
(651677607847)` = ‏factory-test-7 (בית ה-client). ההפעלה דרך gcp-action: ניסיון 1 אושר
ע"י Or אך נכשל על **באג "gcloud כפול"** (הקלט שלי כלל את הקידומת שה-workflow מוסיף בעצמו;
הפקודה לא רצה) → ניסיון 2 בפורמט נכון, ‏Or אישר ✅, ‏execute run 27276629420 הפעיל את
‏drive+docs APIs. **‏✅ הושלם והוכח חי (12:48):** ‏smoke run 27276816404 — ‏**PASS 6/6**
(‏58 כלים; ‏search_drive_files החזיר ‏4,172 בייט של Drive אמיתי); ‏refresh-system-agents
‏(run 27276954106) רענן את 047; הסוכן שלה ענה על שאלת Drive אמיתית — **10 קבצים מהחשבון
המשותף בשמותיהם** (exec 16, success, אומת ב-inspect_n8n_execution). ‏follow-up נוסף:
הקשחת gcp-action נגד קידומת gcloud כפולה (strip אוטומטי בקלאסיפייר).
...
**ממצא חי ‎#2 (סבב רענון ראשון):** אחרי תיקון ה-active הוורקפלו הותקן וה-API הראה
active:true — אבל ‏`/mcp/system-tools` נשאר 404 ("webhook not registered") גם דקות
אחרי: ה-fallback השקט של ההלפר המשותף (PATCH active:true) יכול להדליק את דגל-ה-DB
בלי רישום ראנטיים, ותוצאת ה-/activate לא נראית. תוקן נקודתית בבלוק ה-mcp-server:
הפעלה מפורשת עם לוג מלא + מחזור deactivate→activate על כשל + המתנת-רישום (עד ‏60ש')
באימות-העצמי. ‏(PR ‎#369; ‏באותו סבב גם נפל שער-devplan כי שכחתי לעדכן את הקובץ הזה —
תוקן בדחיפה הזו.)
**ממצא חי ‎#3 (סבב רענון שני, run 27280662987):** ההפעלה המפורשת חשפה את המנגנון —
‏`POST /rest/workflows/:id/activate` **לא קיים** ב-n8n 1.121 (מחזיר 404 + ‏HTML של ה-SPA),
ולכן ה-PATCH של ההלפר רק מדליק דגל-DB בלי לרשום את ה-mcpTrigger. תוקן: הפעלה דרך
ה-**Public API** (deactivate→activate עם X-N8N-API-KEY מ-`n8n-api-key` של המערכת) —
המסלול התכנותי הנתמך. ‏(אגב: לכן וובהוקים רגילים כן עבדו ב-PATCH.) בנוסף תוקנו שתי
טעויות-קלט שלי ל-refresh ‏(paths בפסיקים ויחסי ל-templates/system).
**איטרציה 4 (Or בחר להמשיך לחקור):** גם ‏Public-API ‏200 משאיר 404 — אף מסלול הפעלה
בזמן-ריצה לא רושם mcpTrigger ב-1.121 (וובהוקים רגילים כן; נותר רישום-בעלייה).
...
- **שלב 4 (2026-06-10):** חשבון Google המשותף הורחב — כל הסוכנים רואים מעכשיו גם
  את Drive ואת Docs (קריאה חופשית; כתיבה רק באישור שלך). בדרך אישרת קליק אחד של
