...
**הוכחה תפקודית (באותו שלב):** jq + בדיקת-חיווט בלתי-תלוית-MCP על שני הקבצים; yamllint על המתקין;
סימולציית happy-path + strip-guard (שניהם JSON תקין + topology נכון); CI ירוק. (התנהגות חיה — שלב 5.)

**הערת התקדמות אחרונה:** הושלם. Or בחר כיסוי מלא (Option B). github 15→19, railway 9→13.
נדרש שינוי מתקין (github/railway לא היו Postgres-consumers): הוזרקו @@CRED_POSTGRES_ID@@/@@CHAT_ID@@
+ strip-guard אם אין PG. תוקן באג jq (`index(.name)` → `.name | IN(...)`).

**שינוי תוכנית:** הוגדר מחדש (ראו שלב 2): מ-"3 שאילתות חשיפה" ל-"שכפול אינסטרומנטציה לשני הסיבלינגים".
בנוסף — בניגוד להנחת "בלי שינוי מתקין", github/railway *כן* דרשו נגיעת-מתקין (לא היו Postgres-consumers);
זה ה"שינוי 6" שה-handoff צפה. Or אישר במפורש (Option B) את הכיסוי המלא + נגיעת-המתקין.
...
**Acceptance:**
- [x] תיאור `postgres_named_query` ב-mcp-server.json + ops-agent.json + unknown-agent.json הורחב
      ל-8 השמות (5 קיימים + 3 חדשים) עם gloss קצר לכל אחד.
- [x] גם רשימת-השמות ב-system-prompt של ops-agent עודכנה (עקביות).
- [x] golden + changelog + devplan באותו commit.

**הוכחה תפקודית (באותו שלב):** jq על 3 הקבצים תקין; grep מאשר 8-name list + glosses בכל אחד;
golden + שערי-הזהב ירוקים. (קריאה חיה דרך MCP מוכחת בשלב 5.)

**הערת התקדמות אחרונה:** הושלם. 7 עריכות-טקסט (3 תיאורי-כלי + prompt של ops). אומת jq + grep.
...
**Acceptance:**
- [x] מערכת-טסט לזריקה (factory-test-061) הוקמה ב-reuse mode: provision → register-app → deploy ✅.
- [x] השינוי מהענף הוחל חי דרך `prove-on-test-system.yml` + configure ✅.
- [x] round-trip אמיתי בטלגרם → דרך MCP, כל 4 הקבלות הוכחו חי:
      **tool_trace_recent** = 5 קריאות עם ניסיון→תוצאה (postgres_named_query+railway_readonly+github_readonly,
      status/input/output/duration; קריאה חיה כ-`attempted`); **conversation_transcript** = השיחה המלאה
      עם content (הבדיחה מילה-במילה); **claim_actual_mismatch** = תפס פער אמיתי
      `silently_dropped`/postgres_named_query/`{query:"spend_history_xyz"}` — **בדיוק תבנית #28843**.

**הוכחה תפקודית (באותו שלב):** השלב **הוא** ההוכחה החיה (probe_endpoint / Telegram / MCP).
iterate fix→apply→verify עד ירוק.

**הערת התקדמות אחרונה:** הושלם — כל 4 הקבלות הוכחו חי על factory-test-061 (כולל claim_actual_mismatch
שתפס `silently_dropped` עם ה-input המדויק). db-setup תוקן ואומת. הליטושים (COALESCE לתמלול, חלון-90ש',
grep לאימות db-setup) אומתו חוזר. נותר: שלב 6 — תיעוד, merge, teardown ledger.

**שינוי תוכנית:** נוסף תיקון fix-forward (מאושר ע"י Or) ל-`configure-agent-router.yml`: ב-n8n 2.x
ה-/run של db-setup השמיט `triggerToStartFrom` → "Create Tables" לא רץ → אף טבלה לא נוצרה (false-
positive של HTTP 200). תוקן + נוסף אימות אמיתי של lastNodeExecuted. זה היה חוסם את ההוכחה ושובר את
כל זיכרון-המסד של כל המערכות.
