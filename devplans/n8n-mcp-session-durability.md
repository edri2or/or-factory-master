...
| 5 | הוכחה — unit + smoke מורחב שמאלץ אובדן-session | completed | `services/mcp-server/test/session-store.test.mjs` (חדש), `scripts/n8n-mcp-smoke.py`, `.github/workflows/n8n-mcp-smoke.yml` |
...
**Acceptance:**
- [x] `session-store.test.mjs`: round-trip; secret-safety (אין `x-n8n-key`/`apiKey`/`Bearer` ברשומה המסודרת); TTL פג → miss; `resolveStoredSession` מול fake + fake-זורק.
- [x] `n8n-mcp-smoke.py` מורחב: מאלץ אובדן-session (החלפת-מופע אמיתית של השער) ומוודא התאוששות שקופה — הקריאה מצליחה עם ה-session id המקורי של הלקוח.
- [x] השער האמיתי: ה-smoke החי ירוק אחרי הפריסה ומוכיח את ההתאוששות (run 28386361597, or-edri-4).
...
`datastore.owner`), וה-runtime-SA grant הופך ל-`::warning::` רך — ה-smoke הוא ההוכחה האמיתית.
**2026-06-29 — הושלם.** המענק נקלט (אומת מפלט ה-IAM), הפריסה (#556) הצליחה ליצור את מסד-ה-Firestore
(verify ירוק), ושער-הקבלה החי **עבר**: smoke run 28386361597 על `or-edri-4` — `phase=recover` ירוק,
כלומר session שרד החלפת-מופע אמיתית של השער ושוחזר מהקופסה הקבועה. Layer B מוכח end-to-end.
...
- שלב 5 הושלם — **עמידות ה-session הוכחה חיה**: session שרד החלפת-מופע אמיתית של השער ושוחזר דרך הקופסה הקבועה (Firestore), על המערכת `or-edri-4`. בדרך גם תוקנו שני באגים סמויים שהמבחן חשף (שומר-סף של ה-sidecar, והרשאת ה-broker על מערכת-הבקרה). **הפיתוח הושלם.**
