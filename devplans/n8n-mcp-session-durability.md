...
**הערת התקדמות אחרונה:** 2026-06-29 — b-unit הושלם (12 בדיקות ירוקות). ה-smoke הורחב ל-3 phases
(full/init/recover) + צעד בworkflow שמאלץ החלפת-מופע. נשאר: להריץ את ה-smoke החי אחרי שקבוצה 2 נפרסת — זה השער שסוגר את הפיתוח.

**שינוי תוכנית:** ההתאוששות מוכחת דרך החלפת-מופע אמיתית (revision roll) ולא דרך idle (החלון עכשיו
120 דק', לא מעשי ב-CI) — נאמן יותר לתרחיש ההפקה (deploy/תחזוקה/OOM) שה-store נועד לסגור.
