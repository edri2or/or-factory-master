...
**שורש הבעיה אותר + תוקן:** לא סביבתי — באג Day-0 אמיתי של 2.x. ‏tg-inbound קורא ל-
`tg-vision`/`tg-voice-stt`/`pending-actions-executor` דרך `executeWorkflow` (חיבור ראשי), והם הותקנו
`activate=no`; ב-2.x פרסום הורה שמפנה ל-sub לא-מפורסם נדחה (400) → tg-inbound לא נדלק. התגלה רק עכשיו כי
ה-round-trip האחרון בטלגרם היה על 1.121. **תיקון:** שלושת התתי → `activate=yes` ב-`configure-agent-router.yml`
(golden רוענן). Or דרש לראות את זה עובד בעיניו לפני מיזוג — מיישמים את התיקון על factory-test-053 וחוזרים
לבדיקה בטלגרם. (הבהרה ל-Or: or-tok = כלי-בדיקה שלי שמחובר למערכת אחרת, לא התנגשות בין מערכות.)

**אימות חי הצליח מקצה-לקצה (factory-test-053, n8n 2.25.7):** אחרי התיקון tg-inbound נדלק (active:true,
+ tg-vision/tg-voice-stt/pending-actions-executor). Or שלח "תחקור לעומק על מצב הסוללות במכוניות חשמליות
ב-2026" בטלגרם → (1) ack מיידי "🔎 מתחיל מחקר מעמיק…" (ביצוע ראוטר #26, 0.24ש'); (2) המחקר רץ ברקע
כ-execution עצמאי (#27, fire-and-forget — התחיל באותו רגע, רץ 3.5 דק' עצמאית בזמן שהראוטר כבר סיים);
(3) דוח עברי מקיף (TL;DR + 10 סעיפים + מסקנות) הגיע בנפרד, **מחולק ל-4 הודעות**, עם בלוק "מקורות" של
10 לינקים אמיתיים ששרדו את שומר-הסף (חריג ה-[[SOURCES]]). ביצוע #27 status=success. **כל מטרות הפיתוח
הוכחו חי.** הערה קוסמטית קלה ל-follow-up אופציונלי: המודל הקדים משפט-מטא קצר ("עכשיו אכין דוח…") לפני הדוח.
...
- שלב 2 הושלם — בנינו מערכת-בדיקה טרייה ובדקנו חי. תוך כדי תפסנו באג אמיתי (הדלת-הקדמית של הבוט לא נדלקה
  בגרסה החדשה של n8n כי וורקפלו-עזר לא היו "מפורסמים") — תיקנּו אותו. ואז Or ראה במו עיניו בטלגרם: כתב
  "תחקור לעומק", קיבל מיד "מתחיל לחקור", ואחרי 3.5 דקות הגיע דוח עברי מלא ב-4 הודעות עם 10 מקורות. המחקר
  האסינכרוני עובד מקצה לקצה. נשאר רק לקדם (מיזוג) ולעדכן תיעוד.
