...
| 3 | Deploy + אימות חי | in-progress | `.github/workflows/deploy-mcp-server.yml` (dispatch) |
...
**Acceptance:**
- [ ] PR ירוק → merge ל-`main`
- [ ] `deploy-mcp-server.yml` רץ בהצלחה (build מריץ tsc + node --test)
- [ ] בסשן חדש: `inspect_all_org_resources` מחזיר סיכום משאבים
- [ ] `get_billing_costs` מחזיר `available:false` (לפני warm-up) או `rows`+`totalCost` (אחרי)

**הערת התקדמות אחרונה:** ממתין ל-merge ו-deploy.
