...
שלב 1 קוד-מוכן ונבדק (`npm test` ירוק, 47 בדיקות; `tsc` נקי). שום פריסה עדיין — עוצרים לאישור
Or לפני dispatch בתשלום (שלב 2). מה שמחכה לאישור: הקמת ה-sidecar ב-Cloud Run (סקלה-לאפס, עלות
זניחה) + redeploy של השער.
