...
| 3 | הוכחה חיה על or-edri-4 (תפקודית + E2E) | in-progress | `templates/system/.github/workflows/bootstrap-gmail-oauth.yml`, `e2e-proofs/email-form-intake.json` |
...
**הערת התקדמות אחרונה:** הוחל חי על or-edri-4 (refresh-system-agents, PR or-edri-4 #37 מוזג,
הבוט עצמו אומת). ה-Gmail לא היה מחובר → הרצנו `bootstrap-gmail-oauth` (התחבר אוטומטית, cred
id hMm6VV0KDGIqeQNK) → configure שני התקין את email-form-intake (פעיל, id cTKyxHEOE43jqsPh).
ההרצה הראשונה (exec 644) נכשלה בנוד "List Form Emails" עם "Unable to sign without access token"
— באג חבוי ב-bootstrap (אחסון cred עם access_token ריק + expiry_date:0). תוקן (ראה שינוי תוכנית);
מחילים מחדש ומריצים שוב.

**שינוי תוכנית:** נוסף תיקון ל-`bootstrap-gmail-oauth.yml` — לאחסן access_token תקף + expiry_date
אמיתי (במקום ריק/0), כדי שקרדנציאל ה-googleOAuth2Api יעבוד בקריאות HTTP OAuth2 (חתימה מיידית +
רענון מה-refresh token). באג חבוי שנחשף ע"י הבדיקה החיה (אף workflow לא השתמש ב-Gmail עד עכשיו).
זה מרחיב את הדיף בקובץ provisioning אחד (תחת templates/system/, golden רוענן) — בתוך גבולות הפיתוח.
