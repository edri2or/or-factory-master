...
**הערת התקדמות אחרונה:** בעיצומו (סבב תיקון #3). שלושה באגים אמיתיים תוקנו ברצף ע"י אימות חי:
(1) binary mode `database`→`default`; (2) Redis בלי סיסמה/AOF → סיסמה מילולית + self-healing;
(3) **קריסת-סדר** של ה-main → Redis מוקדם + המתנה ל-`SUCCESS` (אומת על qmode3: אין יותר
`Queue errored`/`Exiting`). אבל ב-qmode3 ה-webhooks עדיין נתקעו — החקירה: ה-main מתזמר תקין
(`Enqueued execution 1/2/3`), אבל שירות ה-worker עלה ב**מצב main** (`n8n ready on port 5678`),
כי start command `n8n worker` הופך ל-`n8n n8n worker` ע"י ה-entrypoint → נפילה ל-main. **תוקן:**
start command → `worker` (ה-entrypoint מוסיף `n8n`). לא באג-גרסה. golden+roadmap+AGENTS עודכנו,
bash/yamllint ירוקים. **נותר:** מערכת טרייה `factory-test-qmode4` → deploy queue_mode=true →
לאמת שה-worker עולה במצב worker וצורך jobs, שה-webhook נענה, ושההודעה נשלחת → לפרק → לסגור.
...
- `factory-test-qmode3` — alive (אימות סבב #2/#3; תפורק בסגירה).
- `factory-test-qmode4` — תיווצר לאימות סבב #3; תפורק בסגירה.
