...
**הערת התקדמות אחרונה (סבב תיקון #4 — 2026-06-01):** 3 באגים תוקנו ואומתו חי (binary, Redis auth/AOF,
קריסת-הסדר). החוסם שנותר — ה-worker עולה כ-main — אובחן: ה-start command של ה-worker **בכלל לא הוחל**
(ב-qmode4 הערך `worker` עלה main עם פריסה אחת; אילו היה מוחל היה קורס על "worker: not found"). שורש:
start command של Railway דורס entrypoint ב-exec form → הערך הנכון `n8n worker` (תבניות Railway הרשמיות),
וה-`WORKER_FIRST_TIME`-gating מיד אחרי serviceCreate מנע redeploy-מחיל. **תוקן:** הערך → `n8n worker`,
מוגדר ב-compare-then-set **לא-מגודר** (כמו Redis) אחרי ה-env, + **המתנה לפריסת worker עד SUCCESS**.
golden+bash+yamllint ירוקים, off-path זהה-בייט. **נותר:** PR ירוק → מזג → מערכת טרייה `factory-test-qmode5`
→ deploy queue_mode=true → לאמת `n8n worker is now ready` + jobs נצרכים + webhook 200 + הודעה נשלחת → לפרק → לסגור.
