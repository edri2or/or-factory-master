...
**הערת התקדמות אחרונה (פאוזה — 2026-06-01):** 3 באגים תוקנו ואומתו חי: (1) binary `database`→`default`;
(2) Redis בלי סיסמה/AOF → סיסמה מילולית + self-healing; (3) **קריסת-סדר** של ה-main → Redis מוקדם +
המתנה ל-`SUCCESS` (אומת על qmode3: אין יותר `Queue errored`/`Exiting`). **חוסם פתוח אחד:** ב-queue
mode ה-webhooks הרשומים נתקעים (timeout) כי שירות ה-**worker עולה כ-n8n main רגיל ולא כ-worker**, ולכן
אף אחד לא צורך את התור. הוכחה: ה-main דוחף תקין (`Enqueued execution 1/2/3`), אבל לוג ה-worker מראה
באנר-main (`n8n ready on ::, port 5678` / `Editor is now accessible`, ולא `n8n worker is now ready`).
**הסיבה האמיתית (מחקר):** start command של Railway על שירות-image **דורס את ה-ENTRYPOINT ב-exec form**.
ניסיון `worker` (qmode4) רץ n8n ב-main (לא קרס על "worker: not found") → **הוכחה שה-start command בכלל לא
הוחל** על הפריסה (ה-worker רץ ברירת-מחדל של ה-image). **הערך הנכון הוא `n8n worker`** (תבניות n8n הרשמיות
של Railway: `n8n worker --concurrency=10`); השינוי שלי ל-`worker` היה שגוי אך חסר-משמעות כי כלום לא הוחל.
**מה שצריך באמת:** (א) להחזיר את ה-start command ל-`n8n worker`, ו-(ב) להבטיח שהוא **מוחל בפועל** —
או redeploy של ה-worker שמרים אותו (כמו אצל Redis) או בנייה דרך `Dockerfile.worker` (`CMD ["worker"]`,
כמו Caddy). **טרם הוחלט/נבדק חי.** ראה הסעיף "פאוזה: מה לחקור" למטה.
