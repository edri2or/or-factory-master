...
**הערת התקדמות אחרונה:** בעיצומו. מוזג #276 → הוקמה `factory-test-qmode` (reuse) → נפרסה עם
`queue_mode=true`: 5 שירותים SUCCESS, ה-worker עלה כ-`n8n worker` והתחבר לתור, ה-n8n הראשי קיבל את
חוזה התור — **הליבה הוכחה חי**. האימות תפס שני באגים (תוקנו ב-PR נוסף): `N8N_DEFAULT_BINARY_DATA_MODE`
`database`→`default`, ו-start command של Redis עבר להזרקת סיסמה מילולית + self-healing (היה נדחק
ע"י Railway). **נותר:** למזג את התיקון → re-deploy על מערכת-הטסט → לאמת חי ש-Redis עם סיסמה+AOF
וריצות מקביליות בלי איבוד → לסגור + Teardown ledger.
