...
|---|---|---|---|
| 1 | רשימת-פטור + מחולל + 14 כרטיסים | pending | `monitoring/workflow-skill-exempt.txt`, `scripts/gen-workflow-skill.sh`, `templates/system/.claude/skills/<name>/SKILL.md` ×14 |
| 2 | שער-CI + חיווט | pending | `scripts/check-workflow-skill-pair.sh`, `playground-tests.yml`, `pipeline-tests.yml`, `provision-system.yml` |
| 3 | תיעוד-עתיד + golden + סגירה | pending | `.claude/commands/build-agent.md` (+מראה), `tests/golden/system/MANIFEST.sha256`, changelog, devplan |
