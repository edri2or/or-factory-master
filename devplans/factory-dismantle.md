...
| 5 | גיזום קוד-מת (3 תת-אצוות לפי סיכון) | **5a in-progress** | **5a** (PR זה): מחיקת 11 workflows מתים + 5 זוגות script+bats + ~21 סקריפטים/קונפיג יתומים + ניקוי 3 רשומות watchdog-registry. ללא נגיעה ב-gateway. **5b** (עתידי): קוד-gateway מת (agent-approval/coordinator-scope/oil-approval ב-services) + ניקוי DISPATCHABLE_WORKFLOWS allowlist ב-tools.ts + מחיקת decommission-test-system.yml/cleanup-orphan-linear-webhooks.yml — מפעיל redeploy. **5c**: destroy הסוד bs-telegram-watermark (Telegram-gated). |
...
- **נותרו: אצווה 5ב** (קוד-gateway מת + allowlist, מפעיל redeploy) **ו-5ג** (destroy bs-telegram-watermark). Phase 3 (קיפול factory-test-8→control) נפרד ו-Or-gated.
