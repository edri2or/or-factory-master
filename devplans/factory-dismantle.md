...
| 4 | פרישת מערכות-הוכחה מתות | **in-progress** | סריקת-אימפקט חיה: GCP (factory-test-21/25) + דלת-WIF של agent-repo **כבר נמחקו**; OIL-approver App **load-bearing** (זהות-מיזוג של ערוץ system-request→or-aios הנשמר) → נדחה לאצווה 5. נותר: פרישת **or-edri-4 + or-edri-base**. שלב 1 (PR זה): הפניית ברירות-מחדל חיות מ-or-edri-4→or-aios (deploy-mcp-server smoke, google-mcp-smoke, workspace-token-audit, publish-static-site). שלב 2: מחיקת שני הריפו-ים דרך propose-repo-delete.yml (Telegram-gated). |
