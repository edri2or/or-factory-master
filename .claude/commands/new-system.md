...
## שלב ב׳½ — הקשחת main (protect-main ruleset)  ·  קריאה/הרשאה, בלי עלות

`bootstrap-system-infra.yml` **במכוון לא** מגן על main (בזמן ה-bootstrap אין עדיין תוכן בריפו). לכן
מיד אחרי ששלב ב׳ מוזג ל-main וה-CI רץ ירוק (כך ששני הקונטקסטים `shellcheck + yamllint` +
`Scan for committed secrets` כבר קיימים ב-GitHub), מפעילים את ההגנה:

1. Dispatch `protect-system-main.yml` עם `system_repo=<SYSTEM_NAME>` (ברירת-המחדל של הקונטקסטים כבר
   תואמת מערכת-אחות נקייה — אין צורך ב-override).
   - web: `mcp__github__actions_run_trigger` (`method=run_workflow`, `owner=edri2or`, `repo=or-factory-master`, `workflow_id=protect-system-main.yml`, `ref=main`, `inputs={system_repo}`).
2. **אמת** בלוג הריצה: `PASS: protect-main ruleset active on edri2or/<SYSTEM_NAME>` וגם `PASS: delete_branch_on_merge=true`.

התוצאה: main של המערכת החדשה מוגן — PR חובה, 2 בדיקות-חובה, בלי force-push/מחיקה — ומחיקת-ענף
אוטומטית דלוקה (מונעת את קונפליקט-הענף-הישן שאחרי squash-merge). זה סוגר במדויק את הפער ש-bootstrap משאיר.
