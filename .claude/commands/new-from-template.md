...
(running in a session on `[your-org]/service-template`)

**Agent behaviour:** Detects `[your-org]/service-template` from the current git
...
**User:** `/new-from-template [your-org]/service-template my-new-api`
(running from any session, not on the template repo)

**Agent behaviour:** Skips auto-detection, uses `[your-org]/service-template`
directly, then follows the same flow. If clone fails all 3 retries, outputs
the remote URL with `gh repo clone [your-org]/my-new-api` as the recovery command.
