...
The deliverable is: a live App on `[your-org]`, five secrets under
`gcp://[your-gcp-project]/secrets/GITHUB_APP_*`, and a workflow run that proves
the full chain.
...
- Your project's ADR for the GitHub App bootstrap via manifest flow — definitive design
...
| `github-actions-sa@[your-gcp-project].iam.gserviceaccount.com` provisioned | `gcloud iam service-accounts describe github-actions-sa@[your-gcp-project].iam.gserviceaccount.com --project=[your-gcp-project]` returns 200 |
| Operator is an admin of the `[your-org]` org | Can view `https://github.com/organizations/[your-org]/settings/apps` |
| Operator has Secret Manager write on `[your-gcp-project]` | `gcloud secrets list --project=[your-gcp-project]` succeeds |
...
- `GITHUB_ORG=[your-org]` (the literal GitHub org **login** — not `[your-domain]`,
...
GITHUB_ORG=[your-org] node scripts/bootstrap-github-app.js
```
...
[your-org]"**. If it shows a personal username, the POST URL is wrong — abort.

After the click, the operator follows the `/installations/new` link the script
prints, selects "Only select repositories → [your-repo]", and clicks
**Install**.

### Step 4 — Verify the runtime chain

Dispatch `github-app-example.yml` via GitHub REST
(`POST /repos/[your-org]/[your-repo]/actions/workflows/github-app-example.yml/dispatches`
...
- A new ADR is **only** warranted if this invocation changes the design in a
  ...
  your project's existing bootstrap ADR already covers the decision.
...
5. **Never attempt to delete a GitHub App via REST** — no such endpoint exists
   ...
   `https://github.com/organizations/[your-org]/settings/apps/<slug>/advanced`.
...
run `GITHUB_ORG=[your-org] node scripts/bootstrap-github-app.js` in Cloud Shell
...
(`https://github.com/organizations/[your-org]/settings/apps/<slug>/advanced`),
asks the operator to confirm which slug (there may be a zombie app alongside the
live one), and waits for them to complete the manual step before proceeding with
a fresh bootstrap.
