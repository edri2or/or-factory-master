...
| GCS state bucket (`tf-state-[your-org-id]`) exists | GCP Console > Cloud Storage |
...
form. Org restriction is enforced by `attribute_condition` alone.
...
`secretVersionAdder` — these roles don't exist at project scope → Error 400.
...
`PAT_SECRETS_WRITE`. All other workflows fetch from GCP SM at runtime.
...
| `GITHUB_PAT_ACTIONS_WRITE` | `actions: write` | this repo | agent autonomous `workflow_dispatch` |

⚠️ **`GITHUB_PAT_ACTIONS_WRITE` is required for full agent autonomy.** Without it, every `workflow_dispatch` requires a human in the GitHub UI — violating the zero-operator-dashboard principle. Create this PAT at the same time as the other three. (See your project's ADRs for the autonomy policy pattern.)
...
|---|---|---|
| `RAILWAY_PROJECT_ID` | `bootstrap.yml` (Stage 1 re-run) | UUID of the [your-railway] project; also stored as GitHub Secret for fast-path idempotency |
| `RAILWAY_ENVIRONMENT_ID` | `bootstrap.yml` (Stage 1 re-run) | UUID of the [your-railway] `production` environment |
...
|---------|---------|-----------|-----|
| API propagation race | Terraform fails at WIF pool creation with `403 API not enabled` | `iam.googleapis.com` not ready immediately after `google_project_service` | Add `time_sleep` (90s) in `main.tf` with `depends_on` on all WIF resources |
| Invalid project-level IAM roles | `Error 400: roles/secretmanager.secretCreator is not supported` | Role doesn't exist at project scope — only at resource level | Remove `google_project_iam_member` for SM roles; use `for_each` in `secrets.tf` |
| Pre-existing secret conflict | `Error 409: Secret X already exists` in `terraform apply` | Secret created out-of-band before Terraform managed it | Add `import {}` block in `secrets.tf` for each pre-existing secret |
| WIF wildcard silent failure | `permission 'iam.serviceAccounts.getAccessToken' denied` everywhere | `attribute.repository/org/*` is a literal string — no real repo matches | Change member to pool-level `principalSet://.../${pool.name}/*` |
...
| `gcloud secrets versions add` fails | Silent failure or `PERMISSION_DENIED` | Secret shell not pre-created in `secrets.tf` / `gcp-bootstrap.yml` not run yet | Pre-declare every secret in `secrets.tf`; run `gcp-bootstrap.yml` first |
| Bootstrap paradox | "Set GitHub Secrets" step fails, `GH_TOKEN` empty | `gcp-bootstrap.yml` can't fetch from GCP SM before SM exists | Keep `PAT_SECRETS_WRITE` as GitHub secret for `gcp-bootstrap.yml` only |
