...
- **`.claude/`** package — slash commands and skills
...
- **`db-setup`** — one-shot manual workflow: creates the 10 Postgres tables (idempotent, `IF NOT EXISTS`) in a single "Create Tables" node.
...
> **Each sub-agent is a folder — `agents/<name>/` is the source of truth** (`agent.yaml` identity +
> routing, `instructions.md` the system prompt, `tools.yaml` the tools). The deterministic compiler
> `scripts/compile-agent.sh` DERIVES the n8n workflow `workflows/n8n/<name>-agent.json` from the folder;
> `configure-agent-router.yml` regenerates each agent from its folder before importing it into n8n.
> So **edit the folder, not the generated JSON** — the committed JSON is a CI-verified build artifact
> (the `check-agent-folder.sh` gate keeps them in sync). Spec: `agents/_spec/agent-folder.spec.md`;
> to add an agent use `/build-agent`. ("Agent" here always means a *sub-agent* — a worker the Router
> dispatches — never the system as a whole.)
...
- **`postgres_named_query`** — a fixed whitelist of **eight** read-only SELECTs (no free SQL): `style_profile_get`, `recent_audit_log`, `pending_actions_open`, `executions_summary_24h`, `spend_total`, `conversation_transcript`, `tool_trace_recent`, `claim_actual_mismatch`.
  - **Reading this bot's own conversation:** `conversation_transcript` returns the full operator↔bot transcript word-for-word; `tool_trace_recent` + `claim_actual_mismatch` expose the per-tool **attempt→result** trace (the bot's "black box" — `claim_actual_mismatch` flags a tool the bot *claimed* it ran but that silently did nothing). These are reachable to the agent as the `postgres_named_query` tool, and **externally** (e.g. from a Claude Code session) via this system's `/mcp/system-tools` MCP endpoint (Bearer `n8n-mcp-server-token`). So a session never has to "discover" how to read the bot's history — it is one named query.
...
- **`google_workspace`** (`ops-agent`) — the shared Google account's (`edriorp38@or-infra.com`) **Gmail + Calendar + Drive + Docs** over MCP (the factory gateway → the Workspace sidecar; gated on `workspace-mcp-bearer`). **Reads are free** — search/read email, list/read calendar events, search/read Drive files and Docs. **Writes are HITL-gated** — sending an email, creating/changing a calendar event, or creating/modifying a Drive file/Doc must FIRST go through `request_write_action` for the operator's ✅; the agent never sends or modifies Google data without that approval. Every call passes `user_google_email="edriorp38@or-infra.com"`. Note: this is the shared **infra** mailbox, not Or's personal Gmail — pointing it at a personal account is a separate, deliberate step.
...
- **HITL for write actions** (`pending_actions`) — **now live**: `request_write_action` + `pending-actions-executor` send the operator a ✅/❌ Telegram card and run the action only on approval. Used today by `google_workspace` writes (send email / calendar / Drive) and by run/activate-workflow ops requests. (`cost_usd` real-spend wiring above remains the open follow-up.)
