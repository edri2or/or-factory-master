...
- **שלב 7 + סגירת הפיתוח (2026-06-11):** פורקו 4 הנותרות, כולן בשערי ✅ של Or: ‏3 התקניות
  ב-`decommission-test-system.yml` עם `GCP_PROJECT_ID` מאומת-מראש (or-tok/factory-test-23,
  ‏tokile/factory-test-18, ‏or-edri-2/factory-test-22 — ‏Railway+DNS+ארכוב לכל אחת), ‏
  ו-project-life-130 (שריד טרום-פקטורי שהשער סירב לו כצפוי) במסלול ייעודי: ‏Railway דרך
  `decommission-railway-projects.yml` (dry-run אומת KEEP 3/DELETE 1 → ריצה אמיתית), והריפו
  דרך `propose-repo-delete.yml` → הקשת ✅ בטלגרם → מחיקה. אימות-סוף בלתי-תלוי: ‏Railway
  ‏**0 פרויקטים**, ‏3 ריפואים `archived:true`, ‏project-life-130 ‏404. ‏**הפיתוח נסגר**
  (`status: completed`). ‏follow-up רשום: העברת ה-OAuth client המשותף מ-`factory-test-7`
  ל-control + איחוד שני ה-clients — פיתוח נפרד באישור Or (עד אז `factory-test-7` נשאר חי).
