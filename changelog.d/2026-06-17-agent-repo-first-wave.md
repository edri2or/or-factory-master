## הגל הראשון של ריפו-סוכנים (agent-repo-first-wave) — שלב 0

פתיחת פיתוח מדורג (`/dev-stage`) לגל הראשון של ריפו-סוכנים מעל התשתית המוכחת
(`agent-repo-product`, סגור): 3 ריפו-סוכנים אמיתיים — **נחשון** (נתב), **נתן** (מחקר/תכנון),
**ספי** (תיעוד) — מחוברים דרך ה-broker. היכולת החדשה: ה-**fan-out של נחשון** (עובד שמצהיר
על תת-עבודה הנשלחת לסוכן אחר דרך ה-broker), שתוכח לבד (capability-first) לפני בנייה.

שלב 0 הוא scaffolding-תיעוד בלבד (קבצי-md), ולכן אף שער-קוד לא מופעל מלבד שער ה-devplan
שזה הקובץ שמשחרר אותו.

- **`devplans/agent-repo-first-wave.md` (חדש, `status: active`)** — תיק-הפיתוח החי: מטרה, 6
  שלבים (0–5), Acceptance + הוכחה-תפקודית פר-שלב, יומן ל-Or. מצהיר במפורש: **יש יכולת חדשה**
  (ה-fan-out), לכן capability-first Step 0 לא מדולג — שלב 1 (spike זרוק) הוא הוכחת-הלבנה הקשה.
  גבול האבטחה: העובד קריאה-בלבד ואין לו מפתח broker — הוא **מצהיר** ובעל-הרשאה (סשן הפקטורי)
  שולח דרך ה-broker הקיים. ה-workflow האוטונומי נדחה לפיתוח נפרד.
- **`docs/capability-cards/firstwave-fanout.md` (חדש, skeleton, `verdict: pending`)** — כרטיס
  היכולת לפי תקדים `agent-broker-handoff.md`: היכולת הגולמית (נחשון מצהיר על תוכנית-פיצול →
  בעל-הרשאה שולח דרך ה-broker → איסוף → נחשון מאחד), ה-fixture המתוכנן, וקריטריוני ה-go/no-go.
  יתמלא בשלב 1 מהריצה החיה.

אין נגיעה ב-`templates/system/**` (שער הזהב לא נדרס), לא ב-`templates/agent-repo/**` (golden
לא נדרס), ולא בקבצי-התנהגות-בוט (שער ה-E2E no-op). אין שינוי קוד-מוצר ב-MVP — רוכבים על
ה-workflows הקיימים.

## הגל הראשון — שלב 1: הוכחת יכולת ה-fan-out (verdict go)

הוכחה capability-first של היכולת החדשה (ה-fan-out של נחשון) על **ריפו-בדיקה זרוק אחד**
(`zz-fanout-spike`) לפני יצירת ריפויי-אמת. ריפויי ה-`zz-` הקודמים נמחקו, אז אור אישר את
המסלול הזול: ריפו אחד שמשחק נתב+שני אחים+מאחד דרך תג-מצב מהימן `[MODE:SPLIT|WORKER|UNIFY]`.

- **`spikes/firstwave-fanout/` (חדש, זרוק)** — `README.md` (runbook) + `nachshon-router.prompt.md`
  (תיעוד ה-prompt). ה-worker-נתב עצמו חי בענף `spike/firstwave-fanout` בלבד (לא ממוזג; התבנית
  ב-main נשארת גנרית — שער הזהב לא נדרס), והוחל על ריפו-הבדיקה דרך `refresh-agent-repo.yml`.
- **`docs/capability-cards/firstwave-fanout.md`** — `verdict: go` עם ראיות מלאות: SPLIT
  (broker `27699916209`) פלט הצהרה תקינה מוגבלת ל-allow-list, fan-out לשתי תת-משימות, ו-UNIFY
  (broker `27700474510`) שמסנתז את שתי התוצאות. כל ה-run-ids מתועדים.
- **תובנת-עיצוב:** fan-out מקבילי לאותו ריפו עושה מרוץ (artifact `agent-result` משותף); בבנייה
  האמיתית האחים הם ריפויים נפרדים → אין התנגשות, והמנהל מסדר שליחות לאותו ריפו.

ללא שינוי קוד-מוצר: רוכבים על `provision-agent-repo.yml` / `refresh-agent-repo.yml` /
`agent-action.yml` הקיימים. אין נגיעה ב-`templates/agent-repo/**` ב-main (golden לא נדרס).

## הגל הראשון — שלב 2: פרובוז 3 הריפויים האמיתיים

נוצרו שלושת ריפו-הסוכנים הפרטיים האמיתיים מעל התשתית המוכחת (אישור-עלות מפורש מאור):
**`nachshon`** (נתב, provision run `27700908287`), **`natan-research`** (מחקר, `27700977619`),
**`sapi-docs`** (תיעוד, `27701033095`). כל אחד נולד פרטי עם 4 הקבצים (`CLAUDE.md`→`AGENTS.md`,
`.mcp.json`, `agent-main.yml`) וקושר לדלת-WIF המשותפת (שלב ה-bind ירוק בכל ריצה). אומת:
`get_repo` (private=true) + `AGENTS.md` מרונדר עם השם והתכלית של כל סוכן.

נוצר דרך ה-`provision-agent-repo.yml` הקיים — אין שינוי קוד. הוכחת לולאת-ה-broker החיה לכל
ריפו מקופלת לשלב 4 (כל סוכן עונה לבד) כדי לא לכפול הרצות Claude.

## הגל הראשון — שלב 3: אופי מותאם (Drive-free) + worker לכל ריפו

החלטת-עיצוב מאור: **Drive בחוץ** — האופי של כל סוכן יושב בריפו שלו (`AGENTS.md`), לא נקרא
מ-Drive. נלקח *עיקרון* התהליך של כל פרסונה ותורגם לעולם הקריאה-בלבד של הריפו.

- **`docs/agent-specs/firstwave/{nachshon,natan,sapi}.md` (חדש)** — האופי המותאם: נחשון (נתב,
  רישום-אחים מוטמע + חוזה `[MODE:SPLIT|WORKER|UNIFY]` + allow-list `natan-research`/`sapi-docs`);
  נתן (מחקר/תכנון על הריפו via Read/Grep/Glob, מבוסס-ראיות, בלי אינטרנט — מסומן כהרחבת-עתיד);
  ספי (טופס 6-בלוקים + סולמות-Admiralty נפרדים + טקסונומיית-MECE, מחזיר רשומה כתוצאה, בלי ארכוב-Drive).
  שומרים את כללי-הבטיחות/אוריינטציה מהתבנית.
- **יושם חי** דרך 3 ענפי-עבודה זרוקים (`wave/persona-{nachshon,natan,sapi}`, לא ממוזגים) הנושאים
  `templates/agent-repo/AGENTS.md` מותאם + `agent-main.yml` (נחשון=worker תלת-מצבי; נתן/ספי=worker
  מעוגן-תפקיד), שהוחלו דרך `refresh-agent-repo.yml` (runs 27704815500/27704822668/27704829541).
  התבנית ב-`main` נשארה גנרית — **שער הזהב (golden) לא נדרס**.

ללא שינוי קוד-מוצר ב-main. הרחבות נדחות מתועדות: תתי-סוכנים פר-סוכן + `agent-fanout.yml` אוטונומי.

## הגל הראשון — שלב 4: כל סוכן עונה לבד (Gate 1)

שלוש לולאות-broker עצמאיות (`agent-action.yml`, phase=propose, green), אחת לכל סוכן — כל אחד
ענה **באופיו**, כל קובץ-תוצאה אומת-יד:
- **נחשון** (broker 27706375600 → `results/s4-nachshon.json`): `mode:"single"`, תיאר את תפקיד-הנתב
  ואת האחים natan-research/sapi-docs.
- **נתן** (broker 27706358638 → `results/s4-natan.json`): תוכנית בת 9 שלבים, שורה-תחתונה תחילה,
  סימון "מבוסס-עקרון/משוער" (פרוטוקול-האמת).
- **ספי** (broker 27706372115 → `results/s4-sapi.json`): רשומת 6-בלוקים מלאה, שני סולמות-Admiralty
  נפרדים (מקור B / מידע 2) + רמת-ביטחון, סיווג MECE לקטגוריה אחת (4 — תהליך/תפעול) עם נימוק-תיקו.

Gate 1 עבר לשלושת הסוכנים. בלי שינוי קוד — רוכבים על ה-broker הקיים.

## הגל הראשון — שלב 5 (סגירה): fan-out + unify על השלושה (Gate 2)

**ההוכחה-הכותרת — הגל הראשון עובד מקצה-לקצה.** בקשה דו-תחומית אמיתית ("חבילה על הודעות commit")
רצה דרך שלושת הריפויים האמיתיים, בניהול-סשן (Option A), שליחה **מסודרת** (לפי תובנת המרוץ משלב 1):
- **SPLIT** — נחשון (broker 27707574956 → `nachshon/results/fw1.json`): `mode:"fanout"`, plan ל-
  `natan-research`+`sapi-docs` (allow-list בלבד) + unify_hint.
- **FAN-OUT** — נתן (broker 27707737197 → `fw1-a.json`, מחקר 7-חלקים) + ספי (broker 27707966860 →
  `fw1-b.json`, רשומת 6-בלוקים מסווגת, סולמות-Admiralty נפרדים).
- **UNIFY** — נחשון (broker 27708278290 → `fw1-final.json`): `mode:"unify"`, מסנתז **באמת** את שניהם.
שרשרת-correlation מלאה אומתה-יד. Gate 2 עבר.

**תובנת-עיצוב:** worker עתיר-code-fences יוצא `status:"unstructured"` (ה-extractor התמים תופס את
בלוק ה-```json הראשון בתשובה) — אך כל התוכן נשמר ב-`answer` והאיחוד עבד. הקשחת ה-extractor = שיפור-עתיד.

**סגירה:** הפיתוח הושלם (כל 6 השלבים). הרחבות נדחות מתועדות בתיק: תתי-סוכנים פר-סוכן, אינטרנט-לנתן/
Drive-לספי, `agent-fanout.yml` אוטונומי, הקשחת-extractor. ללא שינוי קוד-מוצר ב-main לאורך כל הפיתוח.
