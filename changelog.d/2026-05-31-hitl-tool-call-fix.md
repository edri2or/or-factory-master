...
| fix | `templates/system/workflows/n8n/unknown-agent.json` (round 2): prompt hardening alone was insufficient — give `request_write_action` a structured input schema (named fields `target_system`/`action_type`/`target_id`/`human_summary` instead of a single JSON string) and switch the chat model to `anthropic/claude-sonnet-4.5` (temp 0.3) for reliable action-tool calling. Live-verified on factory-test-030 before back-port. |
