...
| feat | `templates/system/workflows/n8n/unknown-agent.json`: switch the general chat agent's model to `openrouter/auto` (OpenRouter's Auto Router picks a cheap or capable model per message). Hybrid strategy — the ops-agent (HITL action path) keeps a fixed strong model for reliability; only the general-chat path is cost-auto-routed. Verified live on factory-test-030. |
