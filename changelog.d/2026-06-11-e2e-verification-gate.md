...
`monitoring/registry-exempt.txt` (פטור watchdog: `e2e-gate.yml` כשער PR-only,
`e2e-verify.yml` ככלי dispatch ידני — תאום ל-`prove-on-test-system.yml`).

## שלב 5 — חיבור לדיסציפלינת /dev-stage

חובת הוכחת-E2E נתפרה לתוך הפיתוח-בשלבים: שדה `הוכחת E2E (artifact)` נוסף לכל שלב בתבנית
ה-devplan + הערת-חובה כללית; קריטריון סגירת-שלב (a.3) נוסף ל-`dev-stage.md` ול-
`dev-stage-factory.md` — שלב שנוגע בקבצי-התנהגות לא נסגר ולא ממוזג בלי
`e2e-proofs/<slug>.json` טרי באותו דיף (השער אוכף; "config imported"/CI-green לא מספיקים).

**Changes:** `templates/devplan/DEVPLAN.template.md`, `.claude/commands/dev-stage.md`,
`.claude/commands/dev-stage-factory.md`, `templates/system/.claude/commands/dev-stage.md`
(mirror), `tests/golden/system/MANIFEST.sha256`.

## שלב 6 — הוכחה חיה על or-edri-4 (כולל retrofit)

הבלם הוכח **חי על or-edri-4 האמיתית** (לבקשת Or — לא על מערכת זרוקה), מה שכלל את ה-retrofit
שלה (PR #5 → merged; הבלם נוסף ל-ruleset של or-edri-4 כ-context חובה). הוכח: (א) `e2e-verify`
שלח הודעות אמיתיות דרך ה-inbound, הבוט ענה נכון (executions 77, 80) → תעודות חתומות;
(ב) PR עם שינוי-התנהגות בלי תעודה → רק "E2E verification gate" אדום → `mergeable_state: blocked`;
(ג) הוספת תעודה תקפה → ירוק → `mergeable_state: clean`. ה-PR להדגמה נסגר בלי מיזוג.
